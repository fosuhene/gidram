...
class Community_m extends My_Model{
  protected $_table_name ='hierachy_community';
  ...
  public $rules = array(
     ...
     'email' => array(
      ...
      public function get_new(){
       ...
       $community->latitude = '';
       $community->longitude = '';
       $community->added_date = '';
       $community->tin = '';
       $community->ghpost_code = '';
       $community->key_code = '';
...
#########################Dashboard Charts#################################################
    ...
    public  function levels_chart(){
      ...
      public  function get_transactions(){
        $query = $this->db->query("SELECT * from vw_transactionlines_tax");
...
public function editcommunityinfo($user_id){
...
public function select_community($id){
  $this->db->select('*');
...
public function updatecommunityinfo($user_id , $data){
  $this->db->where("id", $user_id);
...
public function membertree($parent_key){
  ...
  $this->db->select('*');
  $this->db->from('vwtree_facility');
  $this->db->where('communityID', $parent_key);
  $query = $this->db->get();
  return $query->result_array();
}
...
###############Tree#########################
 public function get_treelevel(){
        ...
        foreach($categories as $p_cat){
            ...
            $categories[$i]->sub = $this->sub_categories($p_cat->communityID);
            $i++;
        }
        return $categories;
    }
...
public function get_content_users($content_users){
 ...
 function fetch_city($state_id)
 {
  ...
  $query = $this->db->get('content_users');
  $output = '<option value="">Select community</option>';
