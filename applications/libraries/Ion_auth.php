...
class Ion_auth
{
	...
	public function is_national($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_national');

		$national_group = $this->config->item('national_group', 'ion_auth');

		return $this->ion_auth_model->in_group($national_group, $id);
	}

	public function is_district($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_district');

		$district_group = $this->config->item('district_group', 'ion_auth');

		return $this->ion_auth_model->in_group($district_group, $id);
	}

		public function is_community($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_community');

		$community_group = $this->config->item('community_group', 'ion_auth');

		return $this->ion_auth_model->in_group($community_group, $id);
	}

		public function is_regional($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_regional');

		$regional_group = $this->config->item('regional_group', 'ion_auth');

		return $this->ion_auth_model->in_group($regional_group, $id);
	}

		public function is_facility($id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_facility');

		$facility_group = $this->config->item('facility_group', 'ion_auth');

		return $this->ion_auth_model->in_group($facility_group, $id);
	}
