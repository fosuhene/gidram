...
class Ion_auth_model extends CI_Model
{
	...
	public function registerclient($identity, $password,$email, $account_name, $manager,$city,$address,$country, $additional_data = [], $groups = [])
	{
		$this->trigger_events('pre_register');
		...
		else if (!$this->config->item('client_group', 'ion_auth') && empty($groups))
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'], ['name' => $this->config->item('client_group', 'ion_auth')], 1)->row();
		if (!isset($query->id) && empty($groups))
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
        ...
        $data1['user_id'] = $ids;
	     ...
	     $this->db->insert($this->tables['content_users'] , $data1);
